<!-- Sync Impact Report
Version change: 0.12.0 → 1.0.0
Modified principles:
- Principle 7 – Delivery Artifacts & Single Source of Truth (now references ratified RTRM)
- Process Guardrails, Risk Tiers & RTRM – updated to reflect ratified matrix
Added sections: None
Removed sections: None
Templates requiring updates:
- ✅ .specify/templates/plan-template.md – Constitution Check already references RTRM; no change required
- ✅ .specify/templates/spec-template.md – Requirements flow unchanged; no change required
- ✅ .specify/templates/tasks-template.md – Evidence prompts still valid; no change required
- ✅ .specify/templates/agent-file-template.md – wording already points to RTRM; no change required
Follow-up TODOs: None
-->

# repo_20251109 Constitution
> **Status**: RATIFIED 2025-11-09 — enforceable governance for the GSFD program.
...
Every increment maintains `spec.md`, `plan.md`, and `tasks.md` in lockstep, with optional `research.md`, `data-model.md`, `quickstart.md`, `contracts/`, and `notes/` folders activated per the risk tier recorded in the plan’s Constitution Check. Use the Risk-Tier Reference Matrix (RTRM) alongside the tier to decide which optional artifacts become mandatory, and record any waivers directly in plan.md. Agents never scaffold Spec-Kit directories manually; humans run the commands and agents update content. Artifacts retire redundant reasoning once conclusions merge back into the canonical files. *Rationale*: A minimal, curated artifact set keeps the doc set slim yet authoritative.
...
The program tracks three tiers to keep gates proportionate, and the Risk-Tier Reference Matrix (RTRM) codifies the required artifacts, validations, and approvals for each:
...
Refer to `.specify/memory/risk_tier_reference_matrix.md` for the canonical artifact/test matrix; document any deviations explicitly inside plan.md.
